...
public sealed partial class MergeVideoViewModel : ObservableObject, IDisposable, ICloseGuard, IFolderDropTarget
{
    ...
    /// <summary>Seeds this feature's options from the settings the shell loaded at startup.</summary>
    public void LoadSettings(UserSettings settings)
    {
        ...
        SourceFileAction = settings.SourceFileAction;
        ExistingOutputAction = settings.ExistingOutputAction;
        CpuThreadLimit = CpuThreadLimits.Contains(settings.CpuThreadLimit)
            ? settings.CpuThreadLimit
            : CpuThreadLimit;
    }

    /// <summary>Builds the settings blob the shell persists (theme preference is its own concern).</summary>
    public UserSettings ExportSettings() => new()
    {
    ...
    /// <summary>Lets the shell ask this feature's own confirmation dialog before closing.</summary>
    public Task<bool> ConfirmCloseAsync() => Task.FromResult(_userPrompt.ConfirmExitWhileBusy(UnfinishedCount));
    ...
    /// <summary>Mirrors the drag-and-drop behaviour: set the folder and scan it immediately.</summary>
